...
Prefer a text-only `Message = {role: system | user | assistant, content: string}` until another content type has a consumer. Do not add ContentPart nesting, tools/vision/audio/embeddings/reasoning enum values, suffix, or stop sequences merely for future autocomplete. This still stands as written, including for audio. ADR 0006 is the one approved exception, and it is exact about what it is not: transcription lives in `ai-providers` as its own request, terminal and inherent `transcribe` method, and added no enum value and no field to `Message`, `CompletionRequest`, or any other serialized contract in either language. It was approved because a shipped consumer was blocked on the boundary, not to reserve a variant; the standing rule is unchanged for everything still hypothetical. Betalingen's last-user constraint is application validation, not automatically a universal rule. Explicitly check source prompt/order semantics before adopting any shared history constraint.
