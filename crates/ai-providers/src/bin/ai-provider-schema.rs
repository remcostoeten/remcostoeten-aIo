//! Generates or checks the committed provider schemas.
//!
//! `cargo run -p ai-providers --features schema-tool --bin ai-provider-schema -- generate`
//! `cargo run -p ai-providers --features schema-tool --bin ai-provider-schema -- generate --check`
//!
...
use ai_providers::schema::{GeneratedSchema, generate_all};
...
fn main() -> ExitCode {
    ...
    if !generate {
        eprintln!("usage: ai-provider-schema generate [--check]");
